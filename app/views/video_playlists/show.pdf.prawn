...
#  open("#{@video_playlist.videos.poster.path(:small)}")
  ...
  @video_playlist.videos.each.with_index do |video, index|
    title = pdf.make_cell(content: video.programme_title.titleize) if video.programme_title.present?
    foreign_language_title = pdf.make_cell(content: video.foreign_language_title.titleize) if video.foreign_language_title.present?
    ...
    pdf.bounding_box([0, pdf.cursor], width: 480, height: 140) do
      ...
      pdf.table(
        ...
        cells.borders = []
        row(0).size = video.programme_title.length > 50 ? 16 : 18
